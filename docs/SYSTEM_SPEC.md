...
- **Targeting (AO-D002).** Ranged (`rangedAllAccess`) units hit any living enemy. Melee: left lane hits enemy left+center, center hits all three, right hits center+right, **front row only**. AO-D013: melee may target the backline only when the target side has no living stack in positions 1-3 (then any living back stack); a single dead front lane exposes nothing. A melee stack with no legal target has nothing to attack (`"<Unit> has no target in reach."`). Taunt overrides enemy targeting. `untargetable` stacks are skipped.
