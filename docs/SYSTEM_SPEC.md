...
- **Targeting (AO-D002).** Ranged (`rangedAllAccess`) units hit any living enemy. Melee: AO-D013 first (front row only while any front stack lives; the backline opens only when positions 1-3 hold no living stack, a single dead front lane exposes nothing), then AO-D021 lane rule: a melee unit reaches only its own or an adjacent lane (left hits left+center, center all three, right center+right; left never reaches right), in both rows. **Softlock guard:** if that leaves a melee stack with no legal target while candidates exist, the nearest lane's candidates become legal (so e.g. a left unit may hit a lone right-lane stack); a stack only has "no target in reach" when the enemy has no targetable stack at all. Taunt overrides enemy targeting. `untargetable` stacks are skipped.
- **Health model (AO-D004).** `maxHp = count x hpPerUnit`. Damage lowers `currentHp` and `count = ceil(currentHp / hpPerUnit)`. Block absorbs first. Healing adds HP, capped at `preBattleMaxCount x hpPerUnit`: each `hpPerUnit` of heal restores one soldier, never above the pre-battle count. A heal never lowers HP or count (a stack already above the cap heals 0). An effect that follows a lethal effect on the same card skips cleanly when its target is dead. Casualties persist into the run army; stacks reduced to 0 are dropped from it at the start of the next run action, so a later recruit/split never reuses their stackId. Relic army-size effects (Royal Banner, Arcane Crystal) update `count`, HP, `startingCount` and `preBattleMaxCount` together.
- **Damage (AO-D018, Heroes 3).** `A = attack x heroEffectiveness (+ Strength/relic flat)`, `D = defense (+ passive/armor) / moraleDefense`; `mod = 1 + min(3, 0.05 x (A-D))` if `A > D`, else `1 - min(0.7, 0.025 x (D-A))`; `raw = max(1, round(A x mod x count x multiplier))` (a landed hit never deals 0), so damage is linear in count. Constants live in `damage.ts` (`ATTACK_ADVANTAGE_*`, `DEFENSE_ADVANTAGE_*`). The old count-tier table (`effectiveCount`) now only scales healing. Hero effectiveness `1 + max(0, stat-10) x 0.02`, capped x1.40 (STR melee, DEX ranged, INT magic, WIS healing). Dodge (player side) `max(0, DEX-10)%`, cap 20%. Max Mana `base + floor(max(0, WIS-10)/2)`, cap 12.
...
- **Start.** Hero (warlord/rogue/mage) with a 12-card deck and a small army (AO-D007), one starting relic (Royal Banner: +6 to the largest stack; Arcane Crystal: +2 max Mana, army -10%). Gold 100, Food 50, Day 1.
...
- **City.** Level 1–3 with 3/5/6 building slots (upgrade 150g / 300g). Recruit swordsman/archer/knight/priest straight into the army (max 6 stacks, same type merges), no garrison (AO-D008). A new type takes the first free slot (AO-D015). Buildings: market, gold mine, mage tower, stable, training hall (+2 max Mana), forge (+2 max Mana), shrine. One permanent doctrine at the Temple.
