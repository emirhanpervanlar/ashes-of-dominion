...
- **Body:** left rail = your 2x3 portrait grid (back column, front column), center = scene panel (empty: no narration text, AO-D023; holds the Drop Card zone for no-target cards and the played card of AO-D011; the effects layer of the next task draws here), right rail = enemy 2x3 grid (front column, back column). **End Turn** is a wide gold bar (`.endturn-bar`) along the bottom of the enemy half of the field (half the field width, right-aligned).
