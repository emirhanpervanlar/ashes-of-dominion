...
function isCity(v: unknown): boolean {
  return isRec(v) && [1, 2, 3].includes(v.level as number) && allOf(v.buildings, isStr) && (v.doctrine === null || isStr(v.doctrine)) && (v.doctrineChosenDay === null || isWhole(v.doctrineChosenDay)) && isWhole(v.mageTowerTier) && v.mageTowerTier <= 3 && isWhole(v.farmTier) && v.farmTier <= 5 && isWhole(v.barracksTier) && v.barracksTier >= 1 && v.barracksTier <= 4;
}
