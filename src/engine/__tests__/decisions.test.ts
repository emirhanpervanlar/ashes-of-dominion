...
describe('AO-001 review gap: card path with no legal target', () => {
  it('a unit card aimed at the backline is rejected while a front stack lives, spending nothing', () => {
...
describe('AO-D002 / AO-D013: melee reaches the backline only when the front row is empty, ranged always', () => {
  ...
  it('a single dead front lane exposes nothing; a lane with no reachable front stack falls back to the nearest front stack even while allies can act (AO-D044)', () => {
    ...
    expect(computeValidTargets(lone, onlyRightNoBack, UNIT_DEFINITIONS.swordsman, withCenterAlly).map((s) => s.position)).toEqual([3]);
  });
